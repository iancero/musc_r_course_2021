...
# Imagine how hard it would be to type out 100 dataset names (correctly)!
```
...
```{r}
files_to_import <- list.files('data/many_datasets', full.names = T)
...
```{r}
df_names <- map_chr(files_to_import, ~ str_extract(.x, 'abcd_[\\w|\\d]*'))
